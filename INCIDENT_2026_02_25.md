...
**Status:** COMPLETE — code changes applied, all tests passing

**Test results:** 25/25 passing (`executor-clob.test.ts`)
- Bug 1 fix: post-slippage tick re-quantization verified (quantize + slippage snaps to 0.001 grid)
- Bug 2 fix: derive-amounts verified (Predict LIMIT orders produce exact price ratios)
...
**Files modified:**
- `packages/agent/src/clob/signing.ts` — `buildOrder()`: post-slippage tick re-quantization (lines 79-94) + derive-amounts for Predict LIMIT (lines 96-111)
- `packages/agent/src/clob/predict-client.ts` — `pricePerShare` two-step multiply (line 366)
- `packages/agent/src/execution/executor.ts` — `attemptUnwind()` systematic vs transient detection
- `packages/agent/src/__tests__/executor-clob.test.ts` — 7 test assertions updated for Bug 3 behavior change
...
**Bug 2 (Predict LIMIT amounts): NOT YET TESTED** — No unwind was needed because both legs filled successfully. This fix would only be exercised when a partial fill triggers an unwind SELL order on Predict. Needs a scenario where Predict fills but Probable doesn't to verify.
