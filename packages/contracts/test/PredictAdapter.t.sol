...
contract PredictMockCTF is IConditionalTokens {
...
contract PredictAdapterTest is Test {
    PredictAdapter adapter;
    MockUSDT usdt;
    PredictMockCTF ctf;
    ...
    bytes32 marketId = keccak256("PREDICT-MARKET-1");
    bytes32 conditionId = keccak256("CONDITION-1");

    function setUp() public {
        usdt = new MockUSDT();
        ctf = new PredictMockCTF(address(usdt));

        adapter = new PredictAdapter(address(ctf), address(usdt));
    ...
    function test_setQuote() public {
        ...
        PredictQuoteHelper.MarketQuote memory q = PredictQuoteHelper.getQuote(adapter, marketId);
    ...
    function test_setQuote_resolvedStatus() public {
        adapter.setQuote(marketId, 0.55e18, 0.45e18, 10_000e6, 8_000e6);
    ...
    function test_buyOutcome_marketNotRegistered() public {
        ...
        adapter.buyOutcome(unregistered, true, 100e6);
    }
    ...
    function test_isResolved_unregisteredMarket() public {
        bytes32 unregistered = keccak256("UNREGISTERED");
        assertFalse(adapter.isResolved(unregistered));
    }
...
library PredictQuoteHelper {
    ...
    function getQuote(PredictAdapter adapter, bytes32 marketId) internal view returns (MarketQuote memory q) {
